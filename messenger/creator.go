...
func FormattedItemAsHTML(c db.Competition) string {
	...
	message = append(message, `<img align="center" border="0" src="`)
	message = append(message, c.LogoURL)
	message = append(message, `" alt="image" title="image" style="outline: none;text-decoration: none;-ms-interpolation-mode: bicubic;clear: both;display: inline-block !important;border: none;height: auto;float: none;width: 100%;max-width: 140px;" width="140" class="v-src-width v-src-max-width" />`)
	...
	if c.Place != "zawody online" {
		placeMessage = fmt.Sprintf("%s <small style=\"color: #8d8d8d\">%s, %s jazdy autem</small>", placeMessage, c.Distance, c.Duration)
	}
