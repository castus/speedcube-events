...
var logoTemplate = `<img align="center" border="0" src="{{.}}" alt="image" title="image" style="outline: none;text-decoration: none;-ms-interpolation-mode: bicubic;clear: both;display: inline-block !important;border: none;height: auto;float: none;width: 100%;max-width: 140px;" width="140" class="v-src-width v-src-max-width" />`
var placeTemplate = `<p style="margin: 0 0 3px">{{.Place}}</p>{{if ne .Place "zawody online"}} <small style="color: #8d8d8d">{{.Distance}}, {{.Duration}} jazdy autem</small>{{end}}</p>`
var eventsTemplate = `{{if ne (len .) 0}}<p style="margin: 0 0 3px">Konkurencje: {{range .}}<img src="https://raw.githubusercontent.com/cubing/icons/main/svgs/event/{{.}}.svg" width="20" height="20" alt="{{.}}" style="vertical-align: bottom;" /> {{end}}{{end}}</p>`
...
//func PrepareMessageForAdded(IDs diff.Differences, competitions []MessengerDTO) string {
//	var message []string
//
//	if IDs.HasAdded() {
//		message = append(message, sectionHeader("Imprezy dodane"))
...
//}
//
//func PrepareMessageForRemoved(IDs diff.Differences, competitions []MessengerDTO) string {
//	var message []string
//
//	if IDs.HasPassed() {
...
//}
//
//func PrepareMessageForChanged(IDs diff.Differences, competitions []MessengerDTO) string {
//	var message []string
//
//	if IDs.HasChanged() {
//		message = append(message, sectionHeader("Zmiany w imprezach"))
...
func FormattedItemAsHTML(c MessengerDTO) string {
	...
	message = append(message, `<div class="u-row-container" style="padding: 0px;background-color: transparent"> <div class="u-row" style="margin: 0 auto;min-width: 320px;max-width: 600px;overflow-wrap: break-word;word-wrap: break-word;word-break: break-word;background-color: transparent;">`)
	message = append(message, `<table border="1" cellpadding="10px" style="margin: 0;border-collapse: collapse;width: 100%;background: white;border: 10px solid #F9DA61;border-top: 1px solid #F9DA61;border-bottom: 5px solid #F9DA61;"> <tbody> <tr> <td style="width: 100px;text-align: center;border-right: 1px solid white;">`)
	message = append(message, logoHTML(c.LogoURL))
	message = append(message, `</td> <td> <table id="u_content_heading_3" style="font-family:'Inter',sans-serif;" role="presentation" cellpadding="0" cellspacing="0" width="100%" border="0"> <tbody> <tr> <td class="v-container-padding-padding" style="overflow-wrap:break-word;word-break:break-word;font-family:'Inter',sans-serif;" align="left"> <!--[if mso]><table width="100%"><tr><td><![endif]-->`)
	...
	if c.HasWCA {
		header = fmt.Sprintf("%s <img src=\"https://www.speedcubing.pl/images/wca_small_logo.png\" width=\"20\" height=\"20\" />", header)
	}
	message = append(message, `<h1 class="v-text-align v-font-size" style="letter-spacing: -0.8px; margin: 0px; color: #000000; line-height: 140%; text-align: left; word-wrap: break-word; font-family:'Inter',sans-serif; font-size: 22px; font-weight: 500;">`)
	message = append(message, header)
	message = append(message, `</h1>`)

	message = append(message, `</h1> <!--[if mso]></td></tr></table><![endif]--> </td> </tr> </tbody> </table> <table id="u_content_text_6" style="font-family:'Inter',sans-serif;" role="presentation" cellpadding="0" cellspacing="0" width="100%" border="0"> <tbody> <tr> <td class="v-container-padding-padding" style="overflow-wrap:break-word;word-break:break-word;/* padding:0px 50px 10px 10px; */font-family:'Inter',sans-serif;" align="left"> <div class="v-text-align v-font-size" style="font-size: 14px; color: #000000; line-height: 140%; text-align: left; word-wrap: break-word;">`)
	message = append(message, fmt.Sprintf("<p style=\"margin: 0 0 3px\">%s</p>", c.Date))

	message = append(message, placeHTML(c.Place, c.Distance, c.Duration))
	...
	if c.CompetitorLimit > 0 {
		registeredNumber := fmt.Sprintf("%d/%d", c.Registered, c.CompetitorLimit)
		if c.Registered == c.CompetitorLimit {
			registeredNumber = fmt.Sprintf("<span style=\"color:#f14561\">%s</span>", registeredNumber)
		}
		message = append(message, fmt.Sprintf("<p style=\"margin: 0 0 3px\">Zarejestrowanych: %s</p>", registeredNumber))
	}
	...
	message = append(message, `</div> </td> </tr> </tbody> </table> </td> </tr> </tbody> </table></div></div>`)

	return strings.Join(message, "\n")
}

func cubeImageForTag(tag string) string {
	return fmt.Sprintf("<img src=\"https://raw.githubusercontent.com/cubing/icons/main/svgs/event/%s.svg\" width=\"20\" height=\"20\" alt=\"%s\" style=\"vertical-align: bottom;\" />", tag, tag)
}

func sectionHeader(header string) string {
	var message = []string{}
	message = append(message, `<div class="u-row-container" style="padding: 0px;background-color: transparent"> <div class="u-row" style="margin: 0 auto;min-width: 320px;max-width: 600px;overflow-wrap: break-word;word-wrap: break-word;word-break: break-word;background-color: transparent;"> <div style="border-collapse: collapse;display: table;width: 100%;height: 100%;background-color: transparent;"> <!--[if (mso)|(IE)]><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td style="padding: 0px;background-color: transparent;" align="center"><table cellpadding="0" cellspacing="0" border="0" style="width:600px;"><tr style="background-color: transparent;"><![endif]--> <!--[if (mso)|(IE)]><td align="center" width="600" class="v-col-border" style="background-color: #f9da61;width: 600px;padding: 0px;border-top: 3px solid #ffffff;border-left: 0px solid transparent;border-right: 0px solid transparent;border-bottom: 0px solid transparent;border-radius: 0px;-webkit-border-radius: 0px; -moz-border-radius: 0px;" valign="top"><![endif]--> <div class="u-col u-col-100" style="max-width: 320px;min-width: 600px;display: table-cell;vertical-align: top;"> <div style="background-color: #f9da61;height: 100%;width: 100% !important;border-radius: 0px;-webkit-border-radius: 0px; -moz-border-radius: 0px;"> <!--[if (!mso)&(!IE)]><!--> <div class="v-col-border" style="box-sizing: border-box; height: 100%; padding: 0px;border-top: 3px solid #ffffff;border-left: 0px solid transparent;border-right: 0px solid transparent;border-bottom: 0px solid transparent;border-radius: 0px;-webkit-border-radius: 0px; -moz-border-radius: 0px;"> <!--<![endif]--> <table id="u_content_heading_4" style="font-family:'Inter',sans-serif;" role="presentation" cellpadding="0" cellspacing="0" width="100%" border="0"> <tbody> <tr> <td class="v-container-padding-padding" style="overflow-wrap:break-word;word-break:break-word;padding:15px 10px 15px 15px;font-family:'Inter',sans-serif;" align="left"> <!--[if mso]><table width="100%"><tr><td><![endif]--> `)
	message = append(message, `<h1 class="v-text-align v-font-size" style="margin: 0px; color: #000000; line-height: 140%; text-align: left; word-wrap: break-word; font-size: 22px; font-weight: 600;">`)
	message = append(message, header)
	message = append(message, `</h1> <!--[if mso]></td></tr></table><![endif]--> </td> </tr> </tbody> </table> <!--[if (!mso)&(!IE)]><!--> </div><!--<![endif]--> </div> </div> <!--[if (mso)|(IE)]></td><![endif]--> <!--[if (mso)|(IE)]></tr></table></td></tr></table><![endif]--> </div> </div> </div>`)

	return strings.Join(message, "\n")
}

func logoHTML(url string) string {
	return parseTemplate(logoTemplate, url)
}
...
func placeHTML(place string, distance string, duration string) string {
	return parseTemplate(placeTemplate, placeData{
		Place:    place,
		Distance: distance,
		Duration: duration,
	})
...
func eventsHTML(events []string) string {
	return parseTemplate(eventsTemplate, events)
}
